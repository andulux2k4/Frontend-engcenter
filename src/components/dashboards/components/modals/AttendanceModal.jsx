...
function AttendanceModal({
  ...
  const fetchClassStudents = async () => {
    ...
    try {
      ...
      const response = await apiService.getClassById(
        token,
      ...
      if (response.success || response.data) {
        ...
        // Mặc định tất cả học sinh đều có mặt (isPresent = true)
        // Chỉ khi tick vào checkbox thì mới đánh dấu vắng
        if (attendanceProp && Object.keys(attendanceProp).length > 0) {
          setAttendance(attendanceProp);
        } else {
          const initial = {};
          formattedStudents.forEach((stu) => {
            initial[stu.id] = true; // Mặc định có mặt
          });
  ...
  const isDateInClassRange = (date) => {
    ...
    const startDate = classData.startDate
      ...
      : classData.startAt
      ? new Date(classData.startAt)
      : classData.openingDate
      ? new Date(classData.openingDate)
      : classData.createdAt
    ...
    const endDate = classData.endDate
      ...
      : classData.endAt
      ? new Date(classData.endAt)
      : classData.closingDate
      ? new Date(classData.closingDate)
      : classData.finishDate
      ? new Date(classData.finishDate)
      : null;
  ...
  const getDateValidationMessage = (date) => {
    ...
    // Kiểm tra ngày bắt đầu (ngày mở lớp)
    if (startDate && selectedDate < startDate) {
      return `❌ Không thể điểm danh: Ngày ${getDayName(
        date
      )} (${selectedDate.toLocaleDateString(
        "vi-VN"
      )}) sớm hơn ngày mở lớp (${startDate.toLocaleDateString("vi-VN")})`;
    }
    ...
    if (endDate && selectedDate > endDate) {
      return `❌ Không thể điểm danh: Ngày ${getDayName(
        ...
        "vi-VN"
      )}) muộn hơn ngày đóng lớp (${endDate.toLocaleDateString("vi-VN")})`;
    }
    ...
    if (!isClassDay(date)) {
      return `❌ Không thể điểm danh: Ngày ${getDayName(
        date
      )} không phải là ngày học của lớp. Lịch học: ${getClassDays().join(
        ", "
      )}`;
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const attendanceData = {
        ...
        note: `Điểm danh lớp ${classData.className} - ${getDayName(
          selectedDate
        )}`,
      };

      const createResponse = await apiService.createAttendanceSession(
        token,
        classId,
        attendanceData
      );
      ...
      if (createResponse.success || createResponse.data) {
        ...
        // Tạo dữ liệu điểm danh cho TẤT CẢ học sinh
        // Mặc định tất cả học sinh đều có mặt (isAbsent = false)
        // Chỉ những học sinh được tick vào checkbox "Vắng" mới có isAbsent = true
        const studentsAttendance = students.map((student) => ({
          studentId: student.id,
          isAbsent: !attendance[student.id], // Nếu attendance[id] = false thì isAbsent = true (vắng)
        }));

        console.log("🔍 Debug điểm danh:");
        console.log(
          "Students:",
          students.map((s) => ({ id: s.id, name: s.name }))
        );
        console.log("Attendance state:", attendance);
        console.log("Final studentsAttendance:", studentsAttendance);
        console.log(
          "Absent students:",
          studentsAttendance.filter((s) => s.isAbsent).map((s) => s.studentId)
        );
        console.log(
          "Present students:",
          studentsAttendance.filter((s) => !s.isAbsent).map((s) => s.studentId)
        ...
        const markResponse = await apiService.markAttendance(
          token,
          attendanceId,
          {
      ...
      if (errorMsg.includes("duplicate key") || errorMsg.includes("E11000")) {
        setError(
          "❌ Đã tồn tại buổi điểm danh cho ngày này. Vui lòng chọn ngày khác hoặc kiểm tra lại danh sách điểm danh."
        );
      } else if (errorMsg.includes("không khớp với lịch học")) {
        setError(
          "❌ Ngày điểm danh không khớp với lịch học của lớp. Vui lòng chọn ngày khác."
        );
      } else {
    ...
    <div className={styles["modal-overlay"]}>
      <div className={styles["modal"]}>
        ...
        <div style={{ marginBottom: "15px" }}>
          ...
          {selectedDate && (
            <div
              style={{
                fontSize: "0.9rem",
                color: isValidAttendanceDate(selectedDate)
                  ? "#27ae60"
                  : "#e74c3c",
                marginTop: "5px",
                padding: "8px",
                borderRadius: "4px",
                backgroundColor: isValidAttendanceDate(selectedDate)
                  ? "#d5f4e6"
                  : "#fdf2f2",
                border: `1px solid ${
                  isValidAttendanceDate(selectedDate) ? "#a8e6cf" : "#f5c6cb"
                }`,
              }}
            >
              {isValidAttendanceDate(selectedDate)
                ? `✅ ${getDayName(selectedDate)} - Ngày học hợp lệ`
                : `❌ ${getDayName(selectedDate)} - ${getDateValidationMessage(
                    selectedDate
                  ).replace("❌ Không thể điểm danh: ", "")}`}
            </div>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div
            style={{ textAlign: "center", padding: "1.5rem 0", color: "#666" }}
          >
              ...
              {students.map((student, idx) => (
                <tr key={student.id || idx}>
                  ...
                  <td>
                    <input
                      type="checkbox"
                      checked={!attendance[student.id]} // Nếu attendance[id] = false thì checkbox được tích (vắng)
