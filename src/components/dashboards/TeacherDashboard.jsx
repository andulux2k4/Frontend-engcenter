  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (!isConnected) {
          console.warn(
            "⚠️ Không thể kết nối server hoặc có CORS issue, sử dụng mock data"
          );
        ...
        if (!token) {
          ...
          setError(
            "Demo Mode: Không thể đăng nhập. Đang hiển thị dữ liệu mẫu."
          );
        ...
        if (activeTab === "classes") {
          ...
          if (response.success) {
            ...
            console.warn(
              "⚠️ API lỗi, fallback về mock data:",
              response.message
            );
            setError(
              `API Error: ${response.message}. Đang hiển thị dữ liệu mẫu.`
            );
            setApiData((prev) => ({ ...prev, classes: getMockClasses() }));
          }
        } else if (activeTab === "schedule") {
          ...
          if (response.success) {
            console.log(
              ...
              Object.keys(response.schedule).length,
              "ngày có lịch"
            );
        ...
        if (
          ...
          setError(
            "Demo Mode: CORS policy chặn kết nối. Đang hiển thị dữ liệu mẫu."
          );
        } else if (
          ...
          setError(
            "Demo Mode: Phiên đăng nhập không hợp lệ. Đang hiển thị dữ liệu mẫu."
          );
      ...
      const fetchSchedule = async () => {
        ...
        try {
          ...
          if (response.success) {
            ...
            console.warn(
              "⚠️ Lỗi API, fallback mock schedule:",
              response.message
            );
            setApiData((prev) => ({ ...prev, schedule: getMockSchedule() }));
          }
  ...
  const generateCalendarDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push({
        ...
        hasSchedule:
          apiData.schedule[dateKey] && apiData.schedule[dateKey].length > 0,
        scheduleCount: apiData.schedule[dateKey]
          ? apiData.schedule[dateKey].length
          : 0,
      });
    }
      ...
      {message.includes("CORS policy") && (
        <div className="cors-help">
          <p style={{ fontSize: "0.9rem", marginTop: "1rem", color: "#666" }}>
            💡 <strong>Để fix CORS issue:</strong>
            <br />
            1. Dừng dev server (Ctrl+C)
            <br />
            2. Chạy lại: <code>npm run dev</code>
            <br />
                ...
                <div className="calendar-monthly-container">
                  ...
                  <div className="calendar-monthly-grid">
                    {calendarDays.map((day, index) => (
                      <div
                        key={index}
                        className={`calendar-monthly-day ${
                          day.isCurrentMonth ? "current-month" : "other-month"
                        } ${day.isToday ? "today" : ""} ${
                          day.hasSchedule ? "has-schedule" : ""
                        } ${
                          selectedDate &&
                          ...
                          <div key={session.id} className="detail-session">
                            ...
                            <div className="session-details">
                              ...
                              <div className="detail-item">
                                <FiUsers className="icon" />
                                <span>
                                  Số học viên: {session.studentCount || "N/A"}
                                </span>
                              </div>
                            </div>
