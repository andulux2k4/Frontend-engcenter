...
function ParentDashboard({ user, onLogout }) {
  ...
  useEffect(() => {
    if (user && user.roleId) {
      ...
      console.log(
        "API URL 1:",
        `https://english-center-website.onrender.com/v1/api/parents/${user.roleId}/children-details`
      );
      console.log(
        "API URL 2:",
        `https://english-center-website.onrender.com/v1/api/parents/my-children-details`
      );

      // 1. Test endpoint với parentId
      apiService
        .getParentChildrenDetails(token, user.roleId)
        .then((res) => {
          ...
          childrenData.forEach((child, index) => {
            ...
            if (
              ...
              typeof child.attendanceSummary.totalClasses === "number"
            ) {
              totalClasses += child.attendanceSummary.totalClasses;
              console.log(
                `Child ${index + 1} attendance summary:`,
                child.attendanceSummary
              );
            }
          });
          ...
          console.log(
            "=== CHILDREN DETAILS API CALL COMPLETED (with parentId) ==="
          );
        })
        .catch((err) => {
          ...
          console.error(
            "API /v1/api/parents/:parentId/children-details error",
            err
          );
        });
      ...
      apiService
        .getUnpaidPayments(token, user.roleId)
        .then((res) => {
          ...
          if (Array.isArray(data.children) && data.children.length > 0) {
            data.children.forEach((child) => {
              if (Array.isArray(child.unpaidPayments)) {
                allUnpaid = allUnpaid.concat(
                  child.unpaidPayments.map((p) => ({
                    ...p,
        ...
        .catch((err) => {
          ...
          console.error("Error response:", err.response);
          console.error(
            "API /v1/api/parents/:parentId/unpaid-payments error",
            err
          );
        });
    }
  ...
  const handleConfirmPayment = async (e) => {
    ...
    // Kiểm tra xem đã có yêu cầu thanh toán chưa
    if (
      selectedPayment.status === "Chờ xác thực" ||
      selectedPayment.status === "Đã thanh toán"
    ) {
      alert(
        "Học phí này đã có yêu cầu thanh toán hoặc đã được thanh toán. Không thể tạo yêu cầu mới."
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (
            errorMessage.includes("Đã có yêu cầu thanh toán đang chờ xử lý")
          ) {
            errorMessage =
              "Đã có yêu cầu thanh toán đang chờ xử lý cho học phí này. Vui lòng chờ admin xác nhận yêu cầu trước đó hoặc liên hệ admin để được hỗ trợ.";
          } else if (errorMessage.includes("Không tìm thấy")) {
            errorMessage =
              "Không tìm thấy thông tin học phí. Vui lòng thử lại sau.";
          } else if (errorMessage.includes("Không có quyền")) {
            errorMessage = "Bạn không có quyền thực hiện thao tác này.";
          }
        } catch (parseError) {
          console.error("Error parsing response:", parseError);
          errorMessage = errorData || "Lỗi không xác định từ server";
        }
      ...
      setUnpaidPayments((prev) => {
        const updated = prev.map((p) => {
          const match =
            (p._id && selectedPayment._id && p._id === selectedPayment._id) ||
      ...
      alert(
        "Đã gửi yêu cầu thanh toán thành công! Vui lòng chờ xác nhận từ admin."
      );
      ...
      let userMessage = error.message || "Lỗi không xác định";

      // Tùy chỉnh thông báo lỗi dựa trên nội dung
      if (userMessage.includes("Đã có yêu cầu thanh toán đang chờ xử lý")) {
        // Đây là lỗi bình thường, không cần hiển thị như lỗi system
        alert(userMessage);
      } else if (
        ...
        alert(`Lỗi khi gửi yêu cầu thanh toán: ${userMessage}`);
      }
    } finally {
